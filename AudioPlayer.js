(function(window, undefined) {
  ...
  var AudioPlayer = (function() {
    ...
    function plActive() {
      if(audio.paused) {
        if(plLi[index]) plLi[index].classList.remove('pl-current');
        return;
      }
      var current = index;
      ...
      if(plLi[current]) plLi[current].classList.add('pl-current');

      imageDiv.innerHTML = ''
      if (playList[current] && playList[current].icon) {
...
pl.innerHTML = 'PlayList is empty';
}
function playToggle() {
if(isEmptyList()) {
return;
...
Let me know if you need to optimize the **canvas resolution sizing settings** or if you want to implement a custom track loading background spinner layout next!
